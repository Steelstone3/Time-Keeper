use crate::{
    controllers::{time_duration_factory::time_duration_factory, time_factory::time_factory},
...
impl TimeKeeper {
    pub fn calculate_end_time(&self) -> String {
        match time_factory(&self.end_time.start_time) {
            Ok(time) => match time_duration_factory(&self.end_time.duration) {
                Ok(duration) => {
...
mod end_time_calculator_should {
    ...
    fn calculate_end_time(
        ...
        // Then
        pretty_assertions::assert_eq!(expected_end_time_result, end_time_result);
    }
}
